...
class UAGGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.netD_B = uag.define_net_dis(opt.input_nc,
                                             ...
                                             init_gain=opt.init_gain,
                                             gpu_ids=opt.gpu_ids)
            self.stego_model = LitUnsupervisedSegmenter.load_from_checkpoint("/content/uaggan/cocostuff27_vit_base_5").cuda()
    ...
    def forward(self):
        ...
        with torch.no_grad():
          self.codeA = self.stego_model(self.real_A)
          self.linear_probsA = torch.log_softmax(self.stego_model.linear_probe(self.codeA), dim=1)
          self.single_imgA = self.real_A[0]
          self.linear_predA = dense_crf(self.single_imgA, self.linear_probsA[0]).argmax(0)
          mask = (self.linear_predA == 7)*1
          self.att_A = torch.tensor(mask).cuda()
          self.res = self.fake_B * self.att_A
          self.unres = self.res + (1-self.att_A)*self.real_A
          fig, ax = plt.subplots(1,3, figsize=(5*3,5))
          ax[0].imshow(self.att_A.cpu().numpy())
          ax[0].set_title("mask")
          ax[1].imshow(unnorm(self.unres.cpu().numpy()).permute(1,2,0))
